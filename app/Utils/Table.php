    ...
    public static function convertTable($data)
    {
        if ($data) {
            ...
            foreach($arrayKeys as $k) {
                $explode = explode('#', $k);
                array_push($headers, [
                    'name' => lcfirst(str_replace(' ','',ucwords(str_replace("_", " ", $explode[0])))),
                    'field' => lcfirst(str_replace(' ','',ucwords(str_replace("_", " ", $explode[0])))),
            ...
            $newData = array_map(function ($item) use ($arrayKeys) {
                ...
                foreach ($arrayKeys as $k) {
                    $explode = explode('#', $k);
                    $newKey = lcfirst(str_replace(' ','',ucwords(str_replace("_", " ", $explode[0]))));
                    // dd($newKey);
